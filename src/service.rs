...
impl NodeService {
    ...
    ///
    /// The reference names an environment file, not a key file, so it is parsed
    /// rather than read whole. This deliberately calls the same reader the
    /// worker health check uses: when these were two implementations, one
    /// parsed `API_SERVER_KEY=` and the other sent the entire file as a bearer
    /// token, so a project passed its health check, was promoted to ready, and
    /// then could not start a single run.
    fn worker_api_key(&self, reference: &Option<String>) -> ServiceResult<String> {
        let path = match reference {
            Some(path) if !path.is_empty() => path,
            _ => return Ok(self.inner.api_key.clone()),
        };
        crate::profiles::read_worker_key(Path::new(path), unsafe { libc::getuid() }).map_err(
...
mod tests {
    ...
    /// The reference names an environment file, and only one line of it is the
    /// key.
    ///
    /// Reading it whole produced a bearer token containing newlines, which
    /// never becomes an HTTP request at all: the project passed its health
    /// check, was promoted to ready, and then failed every run with a transport
    /// error and nothing in either journal to explain it.
    #[test]
    fn the_worker_credential_is_parsed_out_of_the_environment_file() {
        ...
        // Shaped exactly like the file provisioning writes.
        let reference = home.path().join("runtime.env");
        std::fs::write(
            &reference,
            "# Generated by Asterism. One project's Hermes worker.\n\
             HOME=/var/lib/asterism/hermes-projects\n\
             API_SERVER_PORT=18700\n\
             API_SERVER_KEY=6f1d2c3b4a5968778695a4b3c2d1e0ff\n\
             PYTHONUNBUFFERED=1\n",
        )
        .unwrap();
        ...
        let key = service
            .worker_api_key(&Some(reference.to_string_lossy().into_owned()))
            .unwrap();

        assert_eq!(key, "6f1d2c3b4a5968778695a4b3c2d1e0ff");
        assert!(!key.contains('\n'), "a bearer token cannot carry newlines");
        assert!(
            !key.contains("API_SERVER_KEY"),
            "the file is parsed, not sent"
        );
    }

    /// A project bound to the Node-wide endpoint has no reference of its own,
    /// and must keep authenticating with the Node-wide key.
    #[test]
    fn a_project_without_a_reference_still_uses_the_node_wide_key() {
        let home = tempfile::tempdir().unwrap();
        let service = NodeService::new(
            home.path(),
            "http://127.0.0.1:18642",
        ...
        for reference in [None, Some(String::new())] {
            assert_eq!(
                service.worker_api_key(&reference).unwrap(),
                "0123456789abcdef0123456789abcdef"
            );
        }
    }
